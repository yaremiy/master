...
async def evaluate_accessibility(request: URLRequest):
    ...
    try:
        ...
        calculator = ScoreCalculator(
            evaluator.weights, evaluator.metric_weights)

        quality_level = calculator.get_quality_level(result['final_score'])
        quality_description = calculator.get_quality_description(
            result['final_score'])
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Помилка сервера: {str(e)}")
...
async def evaluate_html_accessibility(request: HTMLRequest):
    ...
    print(
        f"🔍 Отримано запит на аналіз HTML контенту (довжина: {len(request.html_content)} символів)")
