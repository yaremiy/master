...
class FormTester {
    ...
    this.invalidTestScenarios = {
      email: [
        ...
        { value: "test@", type: "incomplete", description: "Неповний email" },
        {
          value: "@domain.com",
          type: "missing_local",
          description: "Відсутня локальна частина",
        },
        {
      ...
      number: [
        ...
        },
        {
          ...
          type: "too_large",
          description: "Занадто велике число",
        },
      ],
      tel: [
        { value: "", type: "empty", description: "Порожнє поле" },
      ...
      url: [
        ...
        { value: "http://", type: "incomplete", description: "Неповний URL" },
        {
          ...
          description: "Непідтримуваний протокол",
        },
      ],
      date: [
        { value: "", type: "empty", description: "Порожнє поле" },
  ...
  async testFormErrorBehaviorSystematic(formSelector = "form") {
    ...
    try {
      ...
      this.helpers.log(
        `❌ Помилка систематичного тестування: ${error.message}`,
        "error"
      );
      return this.createSystematicResult(
  ...
  async testFieldSystematic(fieldData) {
    ...
    for (const scenario of testScenarios) {
      this.helpers.log(
        `   📝 Сценарій: ${scenario.description} -> '${scenario.value}'`
      );
      ...
      if (scenarioResult.error_detected) {
        fieldResult.error_detection_summary.html5_api |=
          scenarioResult.signals.html5_api.detected;
        fieldResult.error_detection_summary.aria_support |=
          scenarioResult.signals.aria_support.detected;
        fieldResult.error_detection_summary.dom_changes |=
          scenarioResult.signals.dom_changes.detected;
        fieldResult.error_detection_summary.css_states |=
          scenarioResult.signals.css_states.detected;
      }
    }

    // 5. Крос-перевірка
    fieldResult.overall_support = Object.values(
      fieldResult.error_detection_summary
    ).some(Boolean);
  ...
  async testScenario(fieldElement, scenario) {
    try {
      ...
      this.helpers.log(
        `⚠️ Помилка тестування сценарію: ${error.message}`,
        "warn"
      );
      return {
        scenario: scenario,
  ...
  collectErrorSignals(fieldElement) {
    ...
    try {
      if (fieldElement.validity) {
        ...
        signals.html5_api.validation_message =
    ...
    const fieldContainer =
      fieldElement.closest("div, fieldset, section, form") ||
      ...
      errorSelectors.forEach((selector) => {
        ...
        elements.forEach((el) => {
          ...
          if (text && text.length < 200) {
            // Розумна довжина для повідомлення про помилку
            errorElements.push({
              selector: selector,
              text: text,
  ...
  calculateFieldQualityScore(fieldResult) {
    if (
      !fieldResult.test_scenarios ||
      fieldResult.test_scenarios.length === 0
    ) {
      return 0.0;
    }

    // Середня якість по всіх сценаріях
    const scenarioScores = fieldResult.test_scenarios.map(
      (s) => s.quality_score || 0.0
    );
    const avgScenarioScore =
      scenarioScores.reduce((a, b) => a + b, 0) / scenarioScores.length;

    // Бонус за різноманітність підтримки
    const detectionMethods = Object.values(
      fieldResult.error_detection_summary
  ...
  compileSystematicResults(formSelector, fieldTestResults) {
    ...
    const supportedFields = fieldTestResults.filter(
      (field) => field.overall_support
    ).length;

    // Розрахунок загальної якості форми
    let averageQuality = 0.0;
    if (totalFields > 0) {
      const totalQuality = fieldTestResults.reduce(
        (sum, field) => sum + field.quality_score,
        0
      );
    ...
    const detectionStats = {
      html5_api: fieldTestResults.filter(
        (field) => field.error_detection_summary.html5_api
      ).length,
      aria_support: fieldTestResults.filter(
        (field) => field.error_detection_summary.aria_support
      ).length,
      dom_changes: fieldTestResults.filter(
        (field) => field.error_detection_summary.dom_changes
      ).length,
      css_states: fieldTestResults.filter(
        (field) => field.error_detection_summary.css_states
      ).length,
    };
    ...
    return {
      ...
      field_specific_errors: fieldTestResults.some(
        (field) => field.overall_support
      ),
      detailed_breakdown: {
        error_response: {
        ...
        error_localization: {
          score:
            supportedFields === totalFields
              ? 0.3
              : supportedFields > totalFields / 2
              ? 0.2
              : supportedFields > 0
              ? 0.1
              : 0.0,
        ...
        accessibility: {
          score: Math.min(
            (detectionStats.aria_support / Math.max(totalFields, 1)) * 0.2,
            0.2
          ),
        ...
        message_quality: {
          ...
          description: `Середня якість повідомлень: ${averageQuality.toFixed(
            2
          )}`,
        },
      },
