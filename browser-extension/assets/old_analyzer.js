...
class AccessibilityAnalyzer {
  ...
  init() {
    ...
    if (typeof window.AccessibilityHelpers === "undefined") {
      console.warn(
        "AccessibilityHelpers недоступний, створюємо базову реалізацію"
      );
  ...
  async analyzeAccessibility(options = {}) {
    ...
    try {
      ...
      if (metrics._backendFinalScore !== undefined) {
        ...
        this.helpers.log(
          `Використовуємо backend final score: ${totalScore}`,
          "info"
        );
      } else {
      ...
      this.helpers.log(
        `Аналіз завершено. Загальний скор: ${(totalScore * 100).toFixed(1)}%`
      );
  ...
  async calculateMetrics(pageData, options) {
    this.helpers.log(
      "🌐 Використовуємо Python backend для розрахунку метрик",
      "info"
    );
    ...
    try {
      ...
      const metrics = {
        perceptibility:
          subscores.perceptibility || backendMetrics.perceptibility || 0,
        operability: subscores.operability || backendMetrics.operability || 0,
        understandability:
          subscores.understandability || backendMetrics.understandability || 0,
        localization:
          subscores.localization || backendMetrics.localization || 0,
  ...
  async calculateMetricsFallback(pageData, options) {
    this.helpers.log(
      "🔄 Backend недоступний, повертаємо базові значення",
      "warn"
    );
  ...
  calculateTotalScore(metrics) {
    ...
    Object.entries(weights).forEach(([metric, weight]) => {
      if (metrics[metric] !== undefined) {
        ...
        totalWeight += weight;
        this.helpers.log(
          `${metric}: ${metrics[metric]} * ${weight} = ${contribution}`,
          "info"
        );
      } else {
    ...
    this.helpers.log(
      `Загальний скор: ${totalScore} / ${totalWeight} = ${finalScore}`,
      "info"
    );
    this.helpers.log("================================", "info");
  ...
  identifyIssues(pageData, metrics) {
    ...
    const imagesWithoutAlt = pageData.images.filter(
      (img) =>
        this.helpers.isElementVisible(img) &&
    ...
    // Проблеми кнопок
    const buttonsWithoutNames = pageData.buttons.filter(
      (btn) =>
        this.helpers.isElementVisible(btn) &&
        !this.helpers.getAccessibleName(btn)
    );
  ...
  generateRecommendations(issues, metrics) {
    ...
    if (metrics.perceptibility < 0.7) {
      recommendations.push(
        "Покращіть альтернативні тексти для зображень та контрастність"
      );
    }

    if (metrics.operability < 0.7) {
      recommendations.push(
        "Забезпечте клавіатурну навігацію для всіх інтерактивних елементів"
      );
    }

    if (metrics.understandability < 0.7) {
      recommendations.push(
        "Покращіть підтримку помилок у формах та структуру заголовків"
      );
    }

    if (metrics.localization < 0.7) {
      recommendations.push(
        "Додайте атрибути lang та dir для правильної локалізації"
      );
    }
  ...
  highlightIssues(issues) {
    ...
    issues.forEach((issue) => {
      try {
        ...
        this.helpers.log(
          `Не вдалося підсвітити елемент ${issue.element}`,
          "warn"
        );
      }
    });
